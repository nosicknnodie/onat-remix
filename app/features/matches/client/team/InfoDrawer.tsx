...
export const InfoDrawer = ({
  ...
  const user = payload?.player?.user || payload?.mercenary?.user || null;
  const positions =
    _.compact(
      payload?.player?.user
        ? [user?.position1, user?.position2, user?.position3]
        : payload?.mercenary
          ? [
