...
import type { ContentNode, NewPostDTO, PublishPostResult } from "./types";
...
export async function getPostDetail(postId: string, userId?: string) {
  const post = await (await import("./queries.server")).findPostDetail(postId, userId);
  if (!post) return null;
...
export async function deletePost(postId: string, userId: string) {
  const { softDeletePostByAuthor } = await import("./queries.server");
  try {
...
export async function deleteComment(commentId: string, userId: string) {
  const { softDeleteCommentByAuthor } = await import("./queries.server");
  try {
