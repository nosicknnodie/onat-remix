...
import type { MatchClubWithSummaryRelations } from "~/features/matches/types";
import * as q from "./queries.server";
import type { DashboardData, DashboardMatchInsight, DashboardMom, DashboardPost } from "./types";
...
function buildMatchInsights(
  ...
  grouped.forEach(({ match, matchClubs }) => {
    const summaries = matchSummary.summarizeMatchClubs(matchClubs);
