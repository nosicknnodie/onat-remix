...
interface ICommentSettingsProps {
  comment: PostCommentWithExtras;
  onEditorOpen?: () => void;
}

const CommentSettings = ({ comment, onEditorOpen }: ICommentSettingsProps) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["COMMENTS_QUERY", comment.postId],
      });
    } catch (error) {
      console.error(error);
    }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          {user?.id === comment?.authorId && (
            <>
              <DropdownMenuItem onClick={onEditorOpen}>
